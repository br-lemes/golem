  ...
  id     The id of the sell order you want to buy.`,
	...
	PreRunE: func(cmd *cobra.Command, args []string) error {
		...
		if buyData.order.Type != "sell" {
			return fmt.Errorf("order %q is not a sell order: type %q",
				id, buyData.order.Type)
		}
		if buyFlags.quantity > buyData.order.Quantity {
			return fmt.Errorf(
				"not enough quantity in order %q: required %d, available %d",
				id, buyFlags.quantity, buyData.order.Quantity)
		}
		bank, err := api.MyBank()
		if err != nil {
			return err
		...
		for totalBought < buyFlags.quantity {
			...
			if needBank || buyData.character.Gold < cost {
				...
				if buyData.character.Gold < cost {
					goldData, err := api.MyActionBankWithdrawGold(name,
						cost-buyData.character.Gold)
					if err != nil {
						return err
					}
			...
			var err error
			buyData.character, err =
				routine.Move(buyData.character, "grand_exchange")
			if err != nil {
				return err
			}
...
func init() {
	...
	buyCmd.Flags().IntVarP(&buyFlags.quantity, "quantity", "q", 0,
		"Item quantity")
}
