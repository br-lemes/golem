...
func TestMyActionBankDepositItem(t *testing.T) {
	...
	_, err := client.MyActionBankDepositItem("hero", items)
	if err != nil {
		t.Fatal(err)
	}
}

func TestMyActionBankDepositItemErrors(t *testing.T) {
	for name, client := range map[string]*Client{
		...
		t.Run(name, func(t *testing.T) {
			_, err := client.MyActionBankDepositItem("hero", nil)
			if err == nil {
